...
st.markdown("""
    ...
    /* 2. 側邊欄：中淺藍背景 (科技感) + 強制文字純白 */
    section[data-testid="stSidebar"] {
        background-color: #3d5afe !important; /* 調整為更淺、更有活力的藍色 */
...
# --- 3. 側邊欄 ---
with st.sidebar:
    st.markdown("<h2 style='color:#ffffff;'>💎 戰情控制台</h2>", unsafe_allow_html=True)
...
if ticker:
    ...
    if df is not None:
        ...
        st.markdown(f"<div class='terminal-title'>{ticker} 實時戰情分析 <span style='font-size:16px; font-weight:400; color:#9e9e9e;'>{timeframe} 週期</span></div>", unsafe_allow_html=True)
        ...
        with col_main:
            ...
            fig.add_trace(go.Scatter(x=df.index, y=df['EMA20'], line=dict(color='#3d5afe', width=1.5), name='EMA20'), row=1, col=1)
            fig.add_trace(go.Scatter(x=df.index, y=df['EMA50'], line=dict(color='#ffab00', width=1.5, dash='dot'), name='EMA50'), row=1, col=1)
            ...
            fig.update_xaxes(gridcolor='#f5f5f5', zeroline=False)
            fig.update_yaxes(gridcolor='#f5f5f5', zeroline=False)
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})

        with col_ai:
            st.markdown("<p style='color:#424242; font-weight:bold; font-size:16px; margin-top:5px;'>🤖 AI 決策顧問</p>", unsafe_allow_html=True)
            ...
            st.markdown("<hr style='border-top:1px solid #eeeeee;'>", unsafe_allow_html=True)
